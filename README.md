...
It's strongly advised to use arshiso instead of Arch's official ones, due to the fact that the first is a bit more minimal (650MB) in contrast to the latter (755MB) and contain only the bare-minimum required packages to setup the whole installation process, resulting in a much smoother and fast experience
...
- [`vieb-bin`](https://aur.archlinux.org/packages/vieb-bin/) - Vim Inspired Electron Browser - Vim bindings for the web by design
